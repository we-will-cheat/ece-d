...
io.on('connection', (socket) => {
  console.log('a user connected',socket.id);    
    ...
    socket.on("odd",(obj)=>{
      ...
      while(i<=209)
      {
        var x = process.env["_"+String(i)+"_"];
        console.log(x);
        if(x=="odd")
          ret.push(dict["_"+String(i)+"_"]);
        i++;
      }
      i=46;
      var x = process.env["_"+String(i)+"_"];
        if(x=="odd")
          ret.push(dict["_"+String(i)+"_"]);
      console.log(ret);
      io.to(socket.id).emit("odd",{data:(ret.toString()).replace(/,/g,"<br>")});
    });
    socket.on("even",(obj)=>{
      var ret=[];
      i=154;
      ...
      var x = process.env["_"+String(i)+"_"];
        if(x=="even")
          ret.push(dict["_"+String(i)+"_"]);
      console.log(ret);
      io.to(socket.id).emit("even",{data:(ret.toString()).replace(/,/g,"<br>")});
    });
  });
